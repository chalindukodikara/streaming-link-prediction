...
class Model:
    ...
    def initialize(self, **hyper_params):
        ...
        # Train iterators
        train_gen = FullBatchLinkGenerator(self.graph_train, method="gcn", weighted=True)
        self.train_flow = train_gen.flow(edge_ids_train, edge_labels_train)

        # Test iterators
        test_gen = FullBatchLinkGenerator(self.graph_test, method="gcn", weighted=True)
        self.test_flow = test_gen.flow(edge_ids_test, edge_labels_test)

        # Model defining - Keras functional API + Stellargraph layers
        gcn = GCN(
            layer_sizes=layer_sizes, activations=["relu", "relu"], generator=train_gen, dropout=dropout
        )

        x_inp, x_out = gcn.in_out_tensors()

        prediction = LinkEmbedding(
           activation="relu", method="ip"
        )(x_out)
...
if __name__ == "__main__":
    ...
    logging.info('Training set : accuracy - %s, recall - %s, AUC - %s, F1 - %s, precision - %s', eval[0][1], eval[0][2],
                 eval[0][3], f1_train, eval[0][4])
    logging.info('Testing set : accuracy - %s, recall - %s, AUC - %s, F1 - %s, precision - %s', eval[1][1], eval[1][2],
                 eval[1][3], f1_test, eval[1][4])
