...
class Client:
    ...
    def send_model(self):
        ...
        weights_path = self.weights_path + 'weights_' + str(self.dataset_name) + '_client:' + str(
            self.partition_id) + "_R" + str(self.rounds) + ".npy"
    ...
    def fetch_model(self):
        ...
        logging.info(
            '------------------------- Received aggregated global model from the server -------------------------')
    ...
    def run(self):
            ...
            while not self.STOP_FLAG:
                if self.iteration_number > 0 and self.rounds == 0:
                    ...
                    if self.iteration_number == 1:
                        logging.info(
                            '################################## Next batch processing started: transfer learning is ON ##################################')
                else:
                ...
                if self.STOP_FLAG:
                    ...
                    if self.iteration_number == 0:
                        logging.info(
                            '################ Initial trained model: Final global model evalution after %s rounds ################',
                            self.rounds)
                        ...
                        logging.info(
                            ...
                            round(eval[0][0], 4), round(eval[0][1], 4), round(eval[0][2], 4), round(eval[0][3], 4),
                            f1_train, round(eval[0][4], 4), round(training_start_time - training_end_time, 0))
                        logging.info(
                            'Initially trained model: Testing set : loss - %s, accuracy - %s, recall - %s, AUC - %s, F1 - %s, precision - %s',
                            round(eval[1][0], 4), round(eval[1][1], 4), round(eval[1][2], 4), round(eval[1][3], 4),
                            f1_test, round(eval[1][4], 4))

                    else:
                        logging.info('Batch number %s model fetched from the server', self.iteration_number)
                        logging.info(
                            '################ Batch %s: final global model evalution after %s rounds ################',
                            self.iteration_number, self.rounds)
                        ...
                        logging.info(
                            ...
                            round(testing_start_time - testing_end_time, 0))
                        logging.info(
                    ...
                    if self.iteration_number == 0:
                        logging.info(
                            '------------------------- Initial model training: round %s -------------------------',
                            self.rounds)
                    else:
                        logging.info('------------------------- Batch %s training: round %s -------------------------',
                                     self.iteration_number, self.rounds)
                    ...
                    if self.iteration_number == 0:
                        logging.info('------------------------- Training round %s, loss: %s -------------------------',
                                     self.rounds, str(round(np.mean(losses), 4)))
                        logging.info(
                            '------------------------- Training, round %s: Sent local model to the server -------------------------',
                            self.rounds)
                    else:
                        logging.info('------------------------- Batch round %s, loss: %s -------------------------',
                                     self.rounds, str(round(np.mean(losses), 4)))
                        logging.info(
                            '------------------------- Batch %s, round %s: Sent local model to the server -------------------------',
                            self.iteration_number, self.rounds)
        ...
        logging.info(
            'Result report : Accuracy - %s (%s), Recall - %s (%s), AUC - %s (%s), F1 - %s (%s), Precision - %s (%s)',
        ...
        logging.info(
            'Result report : Accuracy 99th - 90th (%s, %s), Recall 99th - 90th (%s, %s), AUC 99th - 90th (%s, %s), F1 99th - 90th (%s, %s), Precision 99th - 90th (%s, %s), Mean time for a batch - %s (%s) seconds - 99th - 90th (%s, %s)',
            ...
            str(round(np.mean(self.all_test_metric_values[5]), 4)),
            str(round(np.std(self.all_test_metric_values[5]), 4)),
            str(round(np.percentile(self.all_test_metric_values[5], 99), 4)),
            str(round(np.percentile(self.all_test_metric_values[5], 90), 4)))

        logging.info("______________________________________________________________________________________________________ Final Values ______________________________________________________________________________________________________")
        logging.info("##########################################################################################################################################################################################################################")
