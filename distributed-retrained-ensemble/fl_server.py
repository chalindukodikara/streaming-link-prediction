...
class Server:
    ...
    def __init__(self, MODEL, training_rounds, rounds, weights_path, graph_id, MAX_CONN=2, IP=socket.gethostname(),
                 PORT=5000, HEADER_LENGTH=10, iteration_number=0, transfer_learning=False, num_timestamps=0):
    ...
    def update_model(self, new_weights, num_examples):
        ...
        if (len(self.weights) % self.MAX_CONN) == 0 and len(self.weights) != 0:
            ...
            weights_path = self.weights_path + 'weights_' + 'graphID:' + str(self.graph_id) + "_V" + str(
                self.training_cycles) + ".npy"
            ...
            if self.iteration_number != 0:
                ...
                logging.info(
                    "____________________________________ Initial training: round %s finished ____________________________________",
    ...
    def send_model(self, client_socket):
        ...
        if self.iteration_number != 0:
            logging.info('Batch %s, round %s: aggregated global model sent to client-%s at %s:%s',
                         self.iteration_number, self.training_cycles, self.client_ids[client_socket],
                         *self.clients[client_socket])
        elif self.iteration_number == 0 and self.training_cycles != 0:
            logging.info('Initial training round %s: aggregated global model sent to client-%s at %s:%s',
                         self.training_cycles, self.client_ids[client_socket], *self.clients[client_socket])
        elif self.iteration_number == 0 and self.training_cycles == 0:
            logging.info('Randomly initialized global model sent to client-%s at %s:%s', self.client_ids[client_socket],
                         *self.clients[client_socket])
    ...
    def run(self):
        while not self.all_timestamps_finished:
            while not self.stop_flag:
                ...
                for notified_socket in read_sockets:
                    ...
                    if notified_socket == self.server_socket:
                        ...
                        if self.iteration_number != 0:
                            logging.info('Batch %s: recieved model from client-%s at %s:%s', self.iteration_number,
                                         client_id, *self.clients[notified_socket])
                        elif self.iteration_number == 0:
                            logging.info('Initial training: recieved model from client-%s at %s:%s', client_id,
            ...
            if self.iteration_number == 0:
                logging.info(
                    "#################################### Initial Trained final model sent to clients ####################################")
            else:
                logging.info(
                    "#################################### Batch %s: sent the final model to clients ####################################",
                    self.iteration_number)
            # Save weights to use for transfer learning
            weights_path = "./global_weights/" + 'weights_' + 'dataset:' + str(DATASET_NAME) + "_V" + str(
                self.iteration_number) + ".npy"
...
if __name__ == "__main__":
    ...
    logging.info('Server started , graph ID %s, number of clients %s, number of rounds %s, number of timestamps %s',
                 GRAPH_ID, NUM_CLIENTS, TRAINING_ROUNDS, NUM_TIMESTAMPS)
    ...
    logging.info(
        'Training report : Total elapsed time %s seconds, graph ID %s, number of clients %s, training rounds %s, rounds %s, number of timestamps %s',
        round(elapsed_time, 0), GRAPH_ID, NUM_CLIENTS, TRAINING_ROUNDS, ROUNDS, NUM_TIMESTAMPS)
