...
def create_flights(data_edges, data_nodes, training_batch_size, testing_batch_size, initial_timestamp=0, last_timestamp=121): # In days: 20, 2
    ...
    while current_timestamp <= (last_timestamp - testing_batch_size):
        if current_timestamp != initial_timestamp:  # filter training batch
            data_edges_temp = data_edges.loc[data_edges['timestamp'] <= (current_timestamp + testing_batch_size)].loc[data_edges['timestamp'] > (starting_timestamp + testing_batch_size)]
